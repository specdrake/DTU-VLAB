...
def defaultCircuitParams(circuit):
    circuit.V('input', 'in', circuit.gnd, 10@u_V)
    circuit.R(1, 'in', 'out', 1@u_Ω) # not required for simulation
    circuit.X('D1', '1N4148', 'out', circuit.gnd)

def setCircuitParams(V, R, X, circuit):
...
def simulateTemp(circuit):
    ...
    return analyses,temperatures

def plotFunc(circuit):
    ####################################################################################################
    ...
    #r#
    analyses, temperatures = simulateTemp(circuit)
